...
function parse_uploaded_files()
{
	...
	foreach ($_FILES['files']['name'] as $id => $fname) {
		...
		$smt = $parser->parse($content, $static_data, $debug = false); // false for no debug, true for debug
		...
		if ($smt->validate($debug = false)) {
			...
			$validStatements[ $smt->statementId] = $smt;
		} else {
			echo " is invalid!!!!!!!!!\n";
	...
	hidden('parser', $_POST['parser']);
	if ($smt_err == 0) {
		$_SESSION['statements'] = serialize($validStatements);
	}
}
